...
- 루틴은 08:00 / 10:00 / 12:00 KST 세 번 돈다(cron `0 23,1,3 * * *` UTC, 같은 프롬프트, 멱등). 매번 `posts.list`로 세고 부족분만 채운다. 한 번 실행에 사이트당 최대 2편, 총 6편.
- 오늘 publish 가 0편이면 이번 첫 글은 즉시 발행. 나머지는 예약.
- 세 사이트 모두 3/3이면 한 줄만 답하고 끝낸다(토큰 절약).
