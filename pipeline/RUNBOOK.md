...
### 12-4. 블로그스팟(PC) 쪽 적용
- `publish_wave.py` 는 이미 블로그당 첫 편 즉시 + 나머지 예약(gap 간격, 분 랜덤)을 지원한다. 부족한 건 `daily_run.py` 가 블로그당 3편을 만들도록 편수를 올리는 것뿐.
- 드라이브 `_공통_추가규칙_9-4_발행량.md` 에 같은 지침을 적어 뒀다(PC 세션이 읽는다).
